...
public class StatConfiguration : IEntityTypeConfiguration<Stat>
{
    public void Configure(EntityTypeBuilder<Stat> builder)
    {
        ...
        builder.Property(s => s.Label).IsRequired().HasColumnType("nvarchar(max)").HasLocalizedJson();
        builder.Property(s => s.Note).IsRequired().HasColumnType("nvarchar(max)").HasLocalizedJson();
    }
}
